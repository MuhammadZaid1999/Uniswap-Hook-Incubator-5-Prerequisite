...
contract Examples {
    ...
    function checkBool() public pure returns(Boolean b){
        b = Boolean.wrap(true);
    }

    function setWallet() public {
        userWallet = UserWallet.wrap(msg.sender);
    }

    function setWallet1() public {
        userWallet1.push(UserWallet.wrap(msg.sender));
    }

    function setWallet2() public pure returns (UserWallet[] memory) {
        UserWallet[] memory userWallet_1 = new UserWallet[](1);
        userWallet_1[0] = UserWallet.wrap(address(1));
        return  userWallet_1;
    }
}
